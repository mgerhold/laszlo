    ...
    void execute(ScopeStack& scope_stack) const override {
        switch (m_type) {
            case Type::Equals:
                ...
                break;
            case Type::Plus: {
                auto const new_value = m_lvalue->evaluate(scope_stack)->binary_plus(m_rvalue->evaluate(scope_stack));
                m_lvalue->evaluate(scope_stack)->assign(new_value);
                break;
            }
            case Type::Minus: {
                auto const new_value = m_lvalue->evaluate(scope_stack)->binary_minus(m_rvalue->evaluate(scope_stack));
                m_lvalue->evaluate(scope_stack)->assign(new_value);
                break;
            }
            case Type::Asterisk: {
